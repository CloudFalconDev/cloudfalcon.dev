...
export default function BookingTerminal() {
	...
	const handleToggle = () => {
		if (!isOpen) {
			// Track booking terminal opened event
			posthog.capture("booking_terminal_opened", {
				consultation_type: "30min_free",
				source: "booking_terminal_cta",
			});
			setIsOpen(true);
			setIsBooting(true);
			setBootLine(0);
			...
			// Track booking terminal closed event
			posthog.capture("booking_terminal_closed", {
				consultation_type: "30min_free",
				calendly_was_ready: calendlyReady,
			});
			setIsOpen(false);
			setIsBooting(false);
			setCalendlyReady(false);
