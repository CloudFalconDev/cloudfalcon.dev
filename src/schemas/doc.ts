...
export default defineType({
	...
	fields: [
		...
		defineField({
			...
			description:
				"Leave empty for root /docs page. Use 'intro', 'index', or empty string for the main docs landing page.",
			// Slug is optional - empty slug is allowed for root page
		}),
		defineField({
			name: "category",
			title: "Category",
			type: "reference",
			to: [{ type: "category" }],
			description: "Select a category for this documentation page",
		}),
		defineField({
		...
		defineField({
			...
			title: "Sidebar Label",
			type: "string",
			description: "Custom label for sidebar (defaults to title)",
		}),
		defineField({
			name: "description",
			title: "Description",
